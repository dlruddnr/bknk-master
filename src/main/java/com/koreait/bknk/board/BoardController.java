...
//feed 부분--------------------------------------------------------------------------------------------
    ...
    public List<BoardDTO> hotFeedList(@RequestParam("limit") int limit){
    ...
    public int myFeedPage(@RequestParam("limit") int limit, @RequestParam("type") int type, @AuthenticationPrincipal CustomUserPrincipal userDetails){
        BoardDAO boardDAO=new BoardDAO();
        boardDAO.setIuser(userDetails.getUser().getIuser());
